...
3. **L3:** the first guarded `restore_config_revision` vertical slice now has a deterministic registry, expiring owner-scoped proposal, exact-code Web/Zalo approval, idempotent config executor, append-only audit, config-hash verification and bounded compensating rollback. Both runtime flags default off. The action resolves only a verified `config_drift`; it does not claim KPI recovery. Placement, budget-allocation, creative and tracking recovery actions remain unimplemented. Legacy Scenario Lab baseline reset remains test-data-only and cannot authorize L3.
