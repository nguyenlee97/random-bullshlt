...
// Import ảnh qua bundler thay vì URL tuyệt đối: Vite phát hành asset có hash,
// còn bundle design-system (esbuild) nhúng data URI — ảnh hiển thị ở cả hai nơi.
import mascotImageUrl from '../../public/brand/advertising-agent-mascot.png'
import autopilotStrategyShotUrl from '../../public/landing/autopilot-strategy.png'
...
// Block render độc lập (ngoài PublicLanding — ví dụ trong design tool) không có
// IntersectionObserver của trang, nên tự bật các phần tử scroll-reveal để nội dung
// không bị ẩn vĩnh viễn. Trong trang thật, observer của PublicLanding vẫn điều khiển.
function useStandaloneReveal() {
  const ref = useRef(null)
  useEffect(() => {
    const node = ref.current
    if (!node || node.closest('.public-landing-v2')) return
  ...
  return (
    <div className="campaign-stage" aria-label="Mô phỏng hệ thống campaign agentic">
      ...
      <div className="campaign-agent-core">
        ...
        <img src={mascotImageUrl} alt="Robot đại diện cho Advertising Agent" width="720" height="1014" decoding="async" />
...
// Thanh điều hướng công khai. `links` nhận các liên kết hệ sinh thái
// (Ad Server, DMP, Analytics, mock sites...) dạng [{ label, href }].
export function LandingNav({ onEnterAgent, links = [] }) {
  return (
    <header className="landing-nav">
      <a href="/" className="landing-brand" aria-label="Advertising Agent">
        <span><img src={mascotImageUrl} alt="" /></span>
...
// Hero: tuyên bố giá trị + CTA chính. `title` cho phép thử biến thể headline.
  ...
  return (
    <section className="landing-hero-v2" aria-labelledby="landing-title">
      <div className="landing-hero-copy">
        ...
        <h1 id="landing-title">{title ?? <>Một brief.<br /><em>Một campaign<br />hoàn chỉnh.</em></>}</h1>
...
// Block 01 — nỗi đau vận hành campaign mà sản phẩm giải quyết.
export function LandingPain() {
  const sectionRef = useStandaloneReveal()
  return (
    <section ref={sectionRef} className="landing-pain-section" aria-labelledby="pain-title">
...
// Block 02 — cách hoạt động: từ brief đến campaign sẵn sàng, 4 bước.
export function LandingHowItWorks() {
  const sectionRef = useStandaloneReveal()
  return (
    <section ref={sectionRef} className="landing-manifesto" aria-labelledby="manifesto-title">
...
// Block 03 — hai chế độ làm việc: Copilot và Autopilot.
export function LandingModes({ onEnterAgent, onOpenDemo }) {
  const sectionRef = useStandaloneReveal()
  return (
    <section ref={sectionRef} className="landing-mode-section" aria-labelledby="mode-title">
...
// Block 04 — bằng chứng đo được: số liệu evaluation + screenshot sản phẩm thật.
export function LandingProof() {
  const sectionRef = useStandaloneReveal()
  return (
    <section ref={sectionRef} className="landing-proof-section" aria-labelledby="proof-title">
      <div className="landing-pain-heading scroll-reveal" data-scroll-reveal>
      ...
      <div className="landing-proof-card scroll-reveal" data-scroll-reveal>
        ...
        <figure className="landing-proof-shot">
          <img src={autopilotStrategyShotUrl} alt="Màn hình Campaign Autopilot đang dừng tại điểm duyệt chiến lược" loading="lazy" decoding="async" />
...
// Block chốt — lời kêu gọi hành động cuối trang.
...
// Footer — nhận diện thương hiệu + disclosure dữ liệu demo + liên kết.
...
export default function PublicLanding({ onEnterAgent, onOpenDemo }) {
  ...
  return (
    <main ref={landingRef} className="public-landing-v2 h-screen h-[100dvh] overflow-y-auto overflow-x-hidden bg-[#020817] text-white" onPointerMove={moveLight}>
      ...
      <LandingPain />
      <LandingHowItWorks />
      <LandingModes onEnterAgent={onEnterAgent} onOpenDemo={onOpenDemo} />
      <LandingProof />
