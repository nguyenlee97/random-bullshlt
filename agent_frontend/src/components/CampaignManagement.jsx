...
function ConfigEditor({ campaignId, configData, onReload }) {
  ...
  return <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
    ...
    <form onSubmit={save} className="mt-5 grid gap-4 sm:grid-cols-2">
      ...
      <label className="text-xs font-bold text-slate-700 sm:col-span-2">Ngân sách ngày (tùy chọn)<input min="0" type="number" value={form.daily ?? ''} onChange={event => change('daily', event.target.value)} placeholder="Để trống: tự chia tổng ngân sách theo số ngày" className="mt-1.5 h-11 w-full rounded-xl border border-slate-300 px-3 text-sm font-semibold outline-none focus:border-brand-500" /><span className="mt-1 block text-[11px] font-normal text-slate-500">Để trống sẽ hiển thị mức ước tính; không còn hiện 0 đ gây hiểu nhầm.</span></label>
      <label className="text-xs font-bold text-slate-700 sm:col-span-2">Ghi chú revision<input value={note} onChange={event => { setRetryRequestId(''); setNote(event.target.value) }} maxLength={500} placeholder="Ví dụ: điều chỉnh ngân sách sau review tuần 1" className="mt-1.5 h-11 w-full rounded-xl border border-slate-300 px-3 text-sm outline-none focus:border-brand-500" /></label>
      {error && <p className="rounded-xl border border-red-200 bg-red-50 px-3 py-2 text-xs text-red-700 sm:col-span-2">{error}</p>}
      {saved && <p className="rounded-xl border border-emerald-200 bg-emerald-50 px-3 py-2 text-xs text-emerald-700 sm:col-span-2">{saved}</p>}
      <div className="sm:col-span-2"><button disabled={busy} type="submit" className="inline-flex items-center gap-2 rounded-xl bg-brand-600 px-4 py-3 text-xs font-black text-white disabled:opacity-50"><Save className="h-4 w-4" /> {busy ? 'Đang lưu…' : 'Lưu thành revision mới'}</button></div>
    </form>
  </section>
}
