...
function CampaignAgentBubble({ campaignId, title, onNavigate }) {
  ...
  const send = async value => {
    ...
    try {
      const result = await AgentAPI.askCampaignAssistant(campaignId, clean, history)
      setMessages(rows => [...rows, { role: 'assistant', text: result.answer, target: result.target_tab, label: result.target_label, suggestions: result.suggestions || [] }])
  ...
  return <aside className="fixed bottom-3 right-3 z-50 flex max-h-[min(520px,70vh)] w-[calc(100vw-24px)] flex-col overflow-hidden rounded-2xl border border-violet-200 bg-white shadow-[0_26px_80px_rgba(30,41,90,.30)] sm:bottom-5 sm:right-5 sm:w-[min(380px,30vw)] sm:min-w-[310px]" aria-label="Campaign Agent chat">
    ...
    <div className="min-h-0 flex-1 space-y-3 overflow-y-auto bg-slate-50 p-3.5">{messages.map((message, index) => <div key={`${message.role}-${index}`} className={`max-w-[88%] rounded-2xl px-3 py-2.5 text-xs leading-5 ${message.role === 'user' ? 'ml-auto rounded-br-sm bg-brand-600 text-white' : 'rounded-bl-sm border border-slate-200 bg-white text-slate-700'}`}><p className="whitespace-pre-wrap">{message.text}</p>{message.target && <button type="button" onClick={() => { onNavigate(message.target); setOpen(false) }} className="mt-2 inline-flex items-center gap-1 font-black text-brand-700">{message.label}<ChevronRight className="h-3.5 w-3.5" /></button>}{message.role === 'assistant' && message.suggestions?.length > 0 && <div className="mt-2 flex flex-wrap gap-1">{message.suggestions.map(item => <button key={item} type="button" onClick={() => send(item)} className="rounded-full bg-violet-50 px-2 py-1 text-left text-[10px] font-bold leading-4 text-violet-700">{item}</button>)}</div>}</div>)}{busy && <div className="w-fit rounded-2xl border border-slate-200 bg-white px-3 py-2 text-xs text-slate-500">Đang đọc campaign…</div>}</div>
