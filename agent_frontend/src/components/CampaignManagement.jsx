...
function CampaignAgent({ campaign, onOpenHistory }) {
  ...
  return <aside className="min-h-0 border-t border-slate-200 bg-white lg:sticky lg:top-0 lg:h-[calc(100vh-60px)] lg:border-l lg:border-t-0">
    <div className="flex h-full min-h-[390px] flex-col p-4">
      ...
      <div className="mt-auto pt-5"><p className="mb-3 text-[11px] leading-5 text-slate-500">Hỏi về incident và evidence tại Live Evaluation sau khi có kết quả L2 multi-agent. Chat tổng quát cho Campaign Agent chưa có API riêng; lịch sử tạo lập vẫn chỉ đọc.</p>{hasHistory && <button type="button" onClick={() => onOpenHistory(campaign)} className="flex w-full items-center justify-center gap-2 rounded-xl bg-[#10284f] px-4 py-3 text-xs font-black text-white transition hover:bg-[#0b1f40]"><History className="h-4 w-4" /> Mở flow đã hoàn tất</button>}</div>
    </div>
  </aside>
}
...
export default function CampaignManagement({ campaign, loading, onBack, onOpenHistory }) {
  ...
  useEffect(() => {
    if (!campaignId) return undefined
    let active = true
    const refresh = () => AgentAPI.getCampaignEvaluation(campaignId)
      .then(data => { if (active) setEvaluationSummary(data.summary) })
      .catch(() => { if (active) setEvaluationSummary(null) })
  ...
  return <main className="h-full overflow-x-hidden overflow-y-auto overscroll-contain bg-[#eef2f8] text-slate-900">
    ...
    <div className="mx-auto max-w-[1560px] lg:grid lg:grid-cols-[210px_minmax(0,1fr)_340px]">
      ...
      <section className={`${mobilePane === 'agent' ? 'hidden lg:block' : 'block'} min-w-0 p-4 sm:p-6`}>{tab === 'overview' && <section className="mb-5 flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-slate-200 bg-white p-5">
        <div><h2 className="font-bold">Campaign health</h2><p className="mt-1 text-sm text-slate-600">{evaluationSummary
          ? evaluationSummary.open_count > 0 ? evaluationSummary.open_count + ' incident cần theo dõi'
            : evaluationSummary.status === 'healthy' ? 'Không còn tín hiệu cảnh báo ở lần đánh giá gần nhất' : 'Chưa có đánh giá hoàn tất'
          : 'Chưa tải được trạng thái Evaluation'}</p></div>
        <button className="rounded-xl bg-blue-700 px-4 py-2 text-sm font-semibold text-white" onClick={() => setTab('evaluation')}>Xem incident</button>
