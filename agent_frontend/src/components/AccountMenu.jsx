...
import { ChevronDown, Laptop, Loader2, LogIn, LogOut, ShieldCheck, UserRound } from 'lucide-react'
...
export default function AccountMenu({ identity, busy, onLogin, onLogout, onLoadSessions, onRevokeSession, compact = false }) {
  ...
  return (
    <div className="relative">
      <button type="button" onClick={() => setOpen(value => !value)} className="inline-flex max-w-[190px] items-center gap-2 rounded-lg border border-slate-200 bg-white px-2.5 py-1.5 text-xs font-bold text-slate-700 shadow-sm hover:bg-slate-50" aria-expanded={open} aria-label={`Tài khoản ${identity.user?.display_name || ''}`.trim()}>
        <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-brand-100 text-brand-700"><UserRound className="h-3.5 w-3.5" /></span>
      ...
      {open && (
        <div className="absolute right-0 top-full z-[90] mt-2 w-80 max-w-[calc(100vw-2rem)] rounded-2xl border border-slate-200 bg-white p-3 shadow-2xl">
          <div className="rounded-xl bg-slate-50 p-3">
            ...
            <p className="mt-0.5 truncate text-xs text-slate-500">{identity.user?.email}</p>
            <p className={`mt-2 flex items-center gap-1 text-[11px] font-semibold ${identity.user?.email_verified ? 'text-emerald-700' : 'text-amber-700'}`}><ShieldCheck className="h-3 w-3" /> {identity.user?.email_verified ? 'Email đã xác minh' : 'Email chưa xác minh'}</p>
          </div>
